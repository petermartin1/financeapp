...
- [x] **N9. `importWithMappings` isn't atomic across steps. — FIXED.** `ImportRepository` now takes the `Database` and performs all writes (payee inserts, alias inserts with normalize+dedup, transaction inserts, and tag inserts) inside a single `transaction {}`; the read/dedup precomputation stays outside. A failure at any step rolls the whole import back, so there are no orphaned payees/aliases or untagged transactions. Test: `ImportRepositoryTest` (happy path + atomicity: a bad tag reference rolls back the payee and transaction).
...
- [~] **R6. PIN/password in mutable `String` in lock UI. — IMPROVED (inherent Compose limit).** The genuinely sensitive copy — the password `CharArray` used for PBKDF2 derivation — is now zeroed promptly via `PBEKeySpec.clearPassword()` in a `finally`, and `PinUnlockScreen` clears its field after submit. The TextField value itself is unavoidably a Kotlin `String` (Compose holds it immutably), so it can't be zeroed; a `SecureString` wrapper there would be theatrical. Full elimination would require a custom secure text field — out of scope.
...
- [x] **R17. Hardcoded `$` currency prefix. — FIXED (foundation + accounts list).** `formatCurrency`/`CurrencyText`/`BalanceText` now take a `currencyCode` and map it to a symbol via `currencySymbol()` (€, £, ¥, ₹, … with a code-prefixed fallback); the accounts list renders each account's balance in its own `Account.currency` (and the duplicate local formatter there was removed). Test: `CurrencyTextTest` (symbol per code). Note: other screens still pass the default `USD`; threading `account.currency` through them is the same one-line change where a single account's amounts are shown. (Correct minor-units, e.g. 0-decimal JPY, are out of scope — amounts are stored as integer cents app-wide.)
...
- [~] **R34. Koin `single` vs `factory` for ViewModels. — WON'T FIX (single is correct here).** Every VM is obtained once via `koinInject()` in `MainContent` (and `koinInject` itself `remember`s the instance), and the app has no per-screen ViewModel lifecycle. `single` makes each VM live for the session, which is the intended behavior; switching to `factory` would give no benefit and risk creating multiple instances / leaking scopes if injection points multiply. True per-route VM lifecycles would be a larger change (per-destination Koin scopes), not a `single`→`factory` flip.
